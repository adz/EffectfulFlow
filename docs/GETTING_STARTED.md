...
Read this page when you want the fastest path from plain `Result` code to the right FsFlow workflow family for a real application use case.

The core `FsFlow` package contains `Flow` and `AsyncFlow`.
`FsFlow.Net` adds `TaskFlow` for `.NET` task-oriented workflows.
...
Pick the family that matches the honest boundary of the workflow.
Do not choose `TaskFlow` just because one helper somewhere happens to use `Task`.
Do not choose `Flow` if the workflow is mainly async work with sync wrappers around it.
...
FsFlow should sit at the application boundary, not replace ordinary pure code.
...
- the core package should stay free of `.NET Task` concepts
...
- the workflow boundary is `.NET Task`
- task interop is central to the code path
- runtime cancellation should be part of execution
...
That means small sync workflows can stay sync and be reused inside async or task-oriented workflows:
...
[`examples/README.md`](../examples/README.md) for runnable workflows.
